...
public final class SocialViewHelper implements SocialView {
    ...
    private static final int FLAG_DOUBLE_MENTION = 3;
    ...
    @Nullable private Pattern doubleMentionPattern;
    ...
    @NonNull private ColorStateList doubleMentionColors;
    ...
    @Nullable private OnClickListener doubleMentionClickListener;
    ...
    @Nullable private OnChangedListener doubleMentionChangedListener;
    private boolean hashtagEditing;
    private boolean mentionEditing;
    private boolean doubleMentionEditing;
            ...
            if (count > 0 && start > 0) {
                ...
                switch (c) {
                    ...
                    case '@':
                        ...
                        break;
                    default:
                        /*if (s.subSequence(start - 1, start) == "@@"){
                            hashtagEditing = false;
                            mentionEditing = false;
                            doubleMentionEditing = true;
                        }*/
                        ...
                        } else if (doubleMentionChangedListener != null && doubleMentionEditing) {
                            doubleMentionChangedListener.onChanged(SocialViewHelper.this, s.subSequence(
                                    indexOfPreviousNonLetterDigit(s, 0, start - 1) + 1, start
                            ));
                        }
                        break;
            ...
            if (start < s.length()) {
                ...
                switch (s.charAt(index)) {
                    ...
                    case '@':
                        ...
                        break;
                    default:
                        /*if (s.subSequence(start, start + 1) == "@@"){
                            hashtagEditing = false;
                            mentionEditing = false;
                            doubleMentionEditing = true;
                        }*/
    ...
    private SocialViewHelper(@NonNull TextView view, @Nullable AttributeSet attrs) {
        ...
        flags = a.getInteger(R.styleable.SocialView_socialFlags, FLAG_HASHTAG | FLAG_MENTION | FLAG_HYPERLINK | FLAG_DOUBLE_MENTION);
        ...
        doubleMentionColors = a.getColorStateList(R.styleable.SocialView_doubleMentionColor);
    ...
    public Pattern getDoubleMentionPattern() {
        return doubleMentionPattern != null ? doubleMentionPattern : Pattern.compile("@@(\\w+)");
    }
    ...
    public void setDoubleMentionPattern(@Nullable Pattern pattern) {
        if (doubleMentionPattern != null) {
            doubleMentionPattern = pattern;
            recolorize();
        }
    }
    ...
    public boolean isDoubleMentionEnabled() {
        return (flags | FLAG_DOUBLE_MENTION) == flags;
    }
    ...
    public void setDoubleMentionEnabled(boolean enabled) {
        if (enabled != isDoubleMentionEnabled()) {
            flags = enabled ? flags | FLAG_DOUBLE_MENTION : flags & (~FLAG_DOUBLE_MENTION);
            recolorize();
        }
    }
    ...
    public void setDoubleMentionColor(int color) {
        setDoubleMentionColors(ColorStateList.valueOf(color));
    }
    ...
    @Override
    public void setOnDoubleMentionClickListener(@Nullable OnClickListener listener) {
        ensureMovementMethod(listener);
        doubleMentionClickListener = listener;
        recolorize();
    }
    ...
    public void setDoubleMentionTextChangedListener(@Nullable OnChangedListener listener) {
        doubleMentionChangedListener = listener;
    }
    ...
    public List<String> getDoubleMentions() {
        return listOf(view.getText(), getDoubleMentionPattern(), false);
    }
    ...
    private void recolorize() {
        ...
        if (isDoubleMentionEnabled()) {
            spanAll(spannable, getMentionPattern(), new Supplier<CharacterStyle>() {
                        @Override
                        public CharacterStyle get() {
                            return doubleMentionClickListener != null
                                    ? new SocialClickableSpan(doubleMentionClickListener, doubleMentionColors, false)
                                    : new ForegroundColorSpan(doubleMentionColors.getDefaultColor());
                        }
                    }
            );
        }
     ...
     * when {@link } are activated. -> com.hendraanggrian.appcompat.widget.SocialView.OnClickListener
     */
