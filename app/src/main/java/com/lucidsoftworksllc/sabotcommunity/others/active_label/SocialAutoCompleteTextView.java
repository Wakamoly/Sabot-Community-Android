...
public class SocialAutoCompleteTextView extends AppCompatMultiAutoCompleteTextView implements SocialView {
            ...
            if (!TextUtils.isEmpty(s) && start < s.length()) {
                switch (s.charAt(start)) {
                    ...
                    case '@':
                        ...
                        break;
                    /*case s.charAt(1) == '@@':
                        if (getAdapter() != doubleMentionAdapter) {
                            setAdapter(doubleMentionAdapter);
                        }
                        break;*/
                }
                if (s.subSequence(start, 1) == "@@"){
                    if (getAdapter() != doubleMentionAdapter) {
                        setAdapter(doubleMentionAdapter);
                    }
                }
    ...
    private ArrayAdapter doubleMentionAdapter;
    ...
    @Nullable
    public ArrayAdapter getDoubleMentionAdapter() {
        return doubleMentionAdapter;
    }

    public void setDoubleMentionAdapter(@Nullable ArrayAdapter adapter) { doubleMentionAdapter = adapter; }
    ...
    private class CharTokenizer implements Tokenizer {
        private final Collection<String> chars = new ArrayList<>();
